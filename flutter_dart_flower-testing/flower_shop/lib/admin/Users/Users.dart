...
class _AdminUsersState extends State<AdminUsers> {
  ...
  Future<void> deleteUser(String id) async {
    final response = await http.delete(
      Uri.parse('http://10.0.2.2:3001/api/user/delete-user/$id'),
    );
  ...
  Future<void> updateUser(User user) async {
    final response = await http.put(
      Uri.parse('http://10.0.2.2:3001/api/user/update-user/${user.id}'),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
      body: jsonEncode(user),
    ...
    return Scaffold(
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          return ListTile(
            title: Text('User: ${users[index].fullName ?? 'No Name'}'),
                ...
                IconButton(
                  ...
                  onPressed: () async {
                    User? updatedUser = await Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => UserForm(user: users[index]),
                      ),
                    );
                    if (updatedUser != null) {
